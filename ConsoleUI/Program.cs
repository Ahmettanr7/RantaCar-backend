            ...
            if (result.Success == true)
            {
                foreach (var rental in result.Data)
                {
                    Console.WriteLine("Araba : {0}  Müşteri : {1}  Kiraladığı Tarih : {2}  Teslim Tarihi : {3}  Arabanın Günlüğü : {4} Toplam Tutar : {5}",
                        rental.CarName, rental.UserName, rental.RentDate, rental.ReturnDate, rental.DailyPrice, rental.TotalPrice);
                }
            }
            else
            {
        ...
        private static void GetCarDetails()
        {
            ...
            if (result.Success == true)
            {
                foreach (var car in result.Data)
                {
                    ...
                    car.BrandName, car.ColorName, car.ModelYear, car.FuelType, car.DailyPrice);
                }
            }
            else
            {
